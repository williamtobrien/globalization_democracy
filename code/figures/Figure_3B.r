...
# Run m6 from Table 3 with standardized variables
m6 <- plm(equallaw ~ lag_fdi_inflows_stan + lag_fdi_outflows_stan +
             lag_trade_stan + lag_tariff_rate_stan + lag_gdppc_stan +
             lag_gdp_growth_stan + lag_inflation_stan + lag_equallaw + lag2_equallaw,
           data = temp,
...
coefs <- crse6[c("lag_fdi_inflows_stan", "lag_fdi_outflows_stan",
                  "lag_trade_stan", "lag_tariff_rate_stan"), 1]

ses <- crse6[c("lag_fdi_inflows_stan", "lag_fdi_outflows_stan",
                "lag_trade_stan", "lag_tariff_rate_stan"), 2]

sig <- as.numeric(crse6[c("lag_fdi_inflows_stan", "lag_fdi_outflows_stan",
                           "lag_trade_stan", "lag_tariff_rate_stan"), 4] < 0.05)

fig3B_df <- data.frame(var_names, coefs, ses, sig) %>%
